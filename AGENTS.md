...
- Review ownership choices explicitly. For each clone, ask which two owners require independent values after that line. Prefer a borrow when the value only needs to be accessed, a mutable borrow when the original should be changed, and a move when the previous owner no longer needs it. For `Copy` types, prefer ordinary assignment over an explicit `.clone()`.
